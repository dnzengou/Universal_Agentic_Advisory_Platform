...
|-----------|---------------|------------------|
| **Strategic Clarity with Adaptive Execution** | Sun Tzu (terrain/deception), Thucydides (power transitions), Schumpeter (creative destruction) | Define end-state → Identify leverage points → Design multiple paths → Maintain reserves |
| **Power, Institutions, and Legitimacy** | Machiavelli (virtù/institutions), Hobbes (authority/order), Foucault (surveillance/biopolitics) | Authority must be effective, bounded, and transparent to sustain consent |
...
**Options Generation**: No-regrets moves → Contingent strategies → Escalation ladders → Cooperative frameworks
...
1. **Premortem/Postmortem Loops**: "What could fail and why?" → Codify learnings
...
- **Threat Realism (Thucydides/Sun Tzu)**: Capability × Intent analysis; exploit terrain/technology asymmetries
...
def forecast_kpis(df, horizon_months=6):
    ...
    for column in df.columns:
        ...
        if use_sklearn:
            ...
            # Simple confidence band (±1 std of residuals)
...
def render_dashboard():
    ...
    st.title("🎯 Integrated Strategic Advisory Framework")
    ...
    elif page == "🗺️ Supply Chain Heatmap":
        show_supply_chain_heatmap(risk_level)
    elif page == "📋 Strategic Playbooks":
        show_strategic_playbooks()
    elif page == "⚠️ Scenario Modules":
...
def show_supply_chain_heatmap(risk_level):
    ...
    st.markdown("Regional × Node vulnerability matrix (0-100 scale)")
...
def show_strategic_playbooks():
    ...
    with tabs[1]:
        ...
        st.markdown("""
        **Threat Realism (Thucydides/Sun Tzu)**: Capability × intent analysis; exploit terrain/technology asymmetries; deception and denial.
